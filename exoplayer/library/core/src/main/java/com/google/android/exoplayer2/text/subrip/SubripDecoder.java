...
public final class SubripDecoder extends SimpleSubtitleDecoder {
  ...
  protected SubripSubtitle decode(byte[] bytes, int length, boolean reset) {
    ...
    while ((currentLine = removeReplacementChars(subripData.readLine())) != null) {
      ...
      while (TextUtils.isEmpty(currentLine)) {
        currentLine = removeReplacementChars(subripData.readLine());
      }
      ...
      if (matcher.matches()) {
        if (firstIteration && parseTimecode(matcher, 1) != 0L) {
          cueTimesUs.add(0L);
          cueTimesUs.add(0L);
          cues.add(new Cue(Html.fromHtml("<br>Edited at https://subtitletools.com")));
          cues.add(null);
  ...
  private String removeReplacementChars(String currentLine) {
    if (currentLine != null) {
      StringBuilder replacementCharsRemoved = new StringBuilder();

      for (int i = 0; i < currentLine.length(); i++) {
        if (currentLine.charAt(i) != '\u0000' &&
                currentLine.charAt(i) != '\uFFFD') {
          replacementCharsRemoved.append(currentLine.charAt(i));
        }
      }

      return replacementCharsRemoved.toString();
    }
    return null;
  }
