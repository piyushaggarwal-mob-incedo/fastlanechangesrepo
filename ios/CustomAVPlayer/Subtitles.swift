...
public extension AVPlayer {
    ...
    fileprivate var subtitleLabelVerticalConstraint: NSLayoutConstraint? {
        get { return objc_getAssociatedObject(self, &AssociatedKeys.SubtitleHeightKey) as? NSLayoutConstraint }
        set (value) { objc_setAssociatedObject(self, &AssociatedKeys.SubtitleHeightKey, value, objc_AssociationPolicy.OBJC_ASSOCIATION_RETAIN_NONATOMIC) }
    }
    fileprivate var parsedPayload: NSDictionary? {
    ...
    // MARK: - Public methods
    func addSubtitles(parentView:UIView) -> Self {
        
        addSubtitleLabel(parentView:parentView)
        
        return self
        
    }
    
    func open(file filePath: URL, encoding: String.Encoding = String.Encoding.utf8, isPathLocal:Bool ) {
        do {
        var contents:String = ""
        if (isPathLocal){
            let location = filePath.path
            contents = try String(contentsOfFile: location as String)
        }
        else
        {
    ...
    func show(subtitles string: String) {
        ...
        self.addPeriodicTimeObserver(
            ...
            using: { [weak self] (time) -> Void in
                ...
                let subtitle = Subtitles.searchSubtitles(strongSelf.parsedPayload, time.seconds)
                label.text = subtitle?.replacingOccurrences(of: "<[^>]+>", with: "", options: .regularExpression, range: nil)
                // Adjust size
                ...
                self?.subtitleLabel?.isHidden = !Constants.kSTANDARDUSERDEFAULTS.bool(forKey: Constants.kIsCCEnabled)
        })
        
    }
    // MARK: - Private methods
    fileprivate func addSubtitleLabel(parentView:UIView) {
     ...
     //   guard let _ = subtitleLabel else {
            ...
            #if os(iOS)
                subtitleLabel?.font = UIFont.init(name: "\(Utility.sharedUtility.fontFamilyForApplication())-Semibold", size: 14)
            #else
                subtitleLabel?.font = UIFont.init(name: "OpenSans-Semibold", size: 30)
            #endif
            ...
            #else
                subtitleLabelHeightConstraint = NSLayoutConstraint(item: subtitleLabel!, attribute: .height, relatedBy: .equal, toItem: nil, attribute: NSLayoutAttribute.notAnAttribute, multiplier: 1.0, constant: 60.0)
            #endif
            parentView.addConstraint(subtitleLabelHeightConstraint!)
            subtitleLabel?.isHidden = !Constants.kSTANDARDUSERDEFAULTS.bool(forKey: Constants.kIsCCEnabled)
