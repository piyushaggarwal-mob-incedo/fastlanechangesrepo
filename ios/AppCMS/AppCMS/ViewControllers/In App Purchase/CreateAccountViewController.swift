...
class CreateAccountViewController: UIViewController {
    ...
    func showAlertForAlertType(alertType: AlertType, retryAction:UIAlertAction ) {
        ...
        let networkUnavailableAlert:UIAlertController = Utility.presentAlertController(alertTitle: alertTitleString ?? "", alertMessage: alertMessage ?? "", alertActions: [closeAction,retryAction])
    ...
    func showAlert(title:String?,message:String?,alertAction:UIAlertAction) -> Void {
        ...
        let networkUnavailableAlert:UIAlertController = Utility.presentAlertController(alertTitle: title ?? "" , alertMessage: message ?? "", alertActions: [alertAction])
        ...
        if reachability.currentReachabilityStatus() == NotReachable {
            ...
            self.getUserIdWithTransactionWithSuccess(success: { (isSuccess:Bool) in
                if isSuccess == true {
                    ...
                    let retryAlert:UIAlertController = Utility.presentAlertController(alertTitle:Constants.kAccountCreationErrorTitle , alertMessage:Constants.kAccountCreationErrorMessage, alertActions: [closeAction,retryAction])
