...
{
    return [self itemViewAtIndex:self.previousItemIndex];
}
...
#if TARGET_OS_TV
    view.hidden = YES;
    if (index == self.currentItemIndex || index == self.currentItemIndex+1 || index == self.currentItemIndex-1)
    {
        view.hidden = NO;
    }
    else if(self.currentItemIndex == 0)
    {
        [[self itemViewAtIndex:self.numberOfItems-1] setHidden:NO];
    }
    else if(self.currentItemIndex == self.numberOfItems-1)
    {
        [[self itemViewAtIndex:0] setHidden:NO];
    }
    
    if (self.isCarouselFocused) {
        
        if (index == self.currentItemIndex) {
            
//            [Utility addShadowForView:view];
            
            for (id subView in view.subviews) {
                
                if ([subView isKindOfClass:[UIView class]])
                {
                    UIView *borderView = (UIView *)subView;
                    if (borderView.tag == 555) {
...
//            [Utility removeShadowForView:view];
