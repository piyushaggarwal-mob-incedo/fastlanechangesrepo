...
//  SnagFilms
//
//  Created by Abhinav Saldi on 27/03/15.
...
typedef NS_ENUM(NSUInteger, SFPageControlImageType) {
    SFPageControlImageTypeNormal = 1,
    SFPageControlImageTypeCurrent,
    SFPageControlImageTypeMask
};

typedef NS_ENUM(NSUInteger, SFPageControlStyleDefaults) {
    SFPageControlDefaultStyleClassic = 0,
    SFPageControlDefaultStyleModern
};

static SFPageControlStyleDefaults _defaultStyleForSystemVersion;
...
+ (void)initialize
{
    _defaultStyleForSystemVersion = SFPageControlDefaultStyleModern;
}

- (void)_initialize
{
    _numberOfPages = 0;
    _tapBehavior = SFPageControlTapBehaviorStep;
    ...
    _verticalAlignment = SFPageControlVerticalAlignmentMiddle;
...
- (CGFloat)_leftOffset
{
    ...
    CGFloat left = 0.0f;
    switch (_alignment) {
        case SFPageControlAlignmentCenter:
            left = ceilf(CGRectGetMidX(rect) - (size.width / 2.0f));
            break;
        case SFPageControlAlignmentRight:
...
- (CGFloat)_topOffsetForHeight:(CGFloat)height rect:(CGRect)rect
{
    ...
    switch (_verticalAlignment) {
        case SFPageControlVerticalAlignmentMiddle:
            top = CGRectGetMidY(rect) - (height / 2.0f);
            break;
        case SFPageControlVerticalAlignmentBottom:
...
- (void)_setImage:(UIImage *)image forPage:(NSInteger)pageIndex type:(SFPageControlImageType)type
{
    if (pageIndex < 0 || pageIndex >= _numberOfPages) {
        return;
    }
    ...
    switch (type) {
        case SFPageControlImageTypeCurrent:
            dictionary = self.currentPageImages;
            break;
        case SFPageControlImageTypeNormal:
            dictionary = self.pageImages;
            break;
        case SFPageControlImageTypeMask:
...
{
    [self _setImage:image forPage:pageIndex type:SFPageControlImageTypeNormal];
    [self _updateMeasuredIndicatorSizes];
}

- (void)setCurrentImage:(UIImage *)image forPage:(NSInteger)pageIndex
{
    [self _setImage:image forPage:pageIndex type:SFPageControlImageTypeCurrent];;
    [self _updateMeasuredIndicatorSizes];
}

- (void)setImageMask:(UIImage *)image forPage:(NSInteger)pageIndex
{
    [self _setImage:image forPage:pageIndex type:SFPageControlImageTypeMask];
...
- (id)_imageForPage:(NSInteger)pageIndex type:(SFPageControlImageType)type
{
    if (pageIndex < 0 || pageIndex >= _numberOfPages) {
        return nil;
    }
...
{
    return [self _imageForPage:pageIndex type:SFPageControlImageTypeNormal];
}

- (UIImage *)currentImageForPage:(NSInteger)pageIndex
{
    return [self _imageForPage:pageIndex type:SFPageControlImageTypeCurrent];
}

- (UIImage *)imageMaskForPage:(NSInteger)pageIndex
{
    return [self _imageForPage:pageIndex type:SFPageControlImageTypeMask];
}
...
- (void)setStyleWithDefaults:(SFPageControlStyleDefaults)defaultStyle
{
    switch (defaultStyle) {
        case SFPageControlDefaultStyleModern:
            ...
            break;
        case SFPageControlDefaultStyleClassic:
        default:
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (SFPageControlTapBehaviorJump == self.tapBehavior) {
