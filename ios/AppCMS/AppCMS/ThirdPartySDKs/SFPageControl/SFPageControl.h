...
typedef NS_ENUM(NSUInteger, SFPageControlAlignment) {
    SFPageControlAlignmentLeft = 1,
    SFPageControlAlignmentCenter,
    SFPageControlAlignmentRight
};

typedef NS_ENUM(NSUInteger, SFPageControlVerticalAlignment) {
    SFPageControlVerticalAlignmentTop = 1,
    SFPageControlVerticalAlignmentMiddle,
    SFPageControlVerticalAlignmentBottom
};

typedef NS_ENUM(NSUInteger, SFPageControlTapBehavior) {
    SFPageControlTapBehaviorStep	= 1,
    SFPageControlTapBehaviorJump
};
...
@property (nonatomic) SFPageControlAlignment alignment					UI_APPEARANCE_SELECTOR; // deafult is Center
@property (nonatomic) SFPageControlVerticalAlignment verticalAlignment	UI_APPEARANCE_SELECTOR;	// deafult is Middle
...
@property (nonatomic) SFPageControlTapBehavior tapBehavior;	// SFPageControlTapBehaviorStep provides an increment/decrement behavior exactly like UIPageControl. SFPageControlTapBehaviorJump allows specific pages to be selected by tapping their respective indicator. Default is SFPageControlTapBehaviorStep
...
// SFPageControl mirrors UIPageControl's standard accessibility functionality by default.
// Basically, the accessibility label is set to "[current page index + 1] of [page count]".

// SFPageControl extends UIPageControl's functionality by allowing you to name specific pages. This is especially useful when using
